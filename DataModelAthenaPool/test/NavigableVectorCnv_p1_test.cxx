// $Id$
/**
 * @file  DataModelAthenaPool/test/NavigableVectorCnv_p1_test.cxx
 * @author scott snyder
 * @date Dec, 2009
 * @brief Regression test for @c NavigableVectorCnv_p1.
 */
...
void test1()
{
  ...
  NavigableVectorCnv_p1<VNI>::PersNavigableVector_t p1;
  cnv.transToPers (vni1, p1, log);

  assert (p1.size() == 2);
  assert (p1.m_navigableSizes[0] == 2);
  assert (p1.m_navigableSizes[1] == 3);
  assert (p1.m_links.m_links.size() == 3);
  assert (p1.m_links.m_links[0].m_SGKeyHash == 152280269);
  assert (p1.m_links.m_links[1].m_SGKeyHash == 793080788);
  assert (p1.m_links.m_links[2].m_SGKeyHash == 887336650);
  assert (p1.m_links.m_elementRefs.size() == 5);
  ...
  assert (p1.m_links.m_elementRefs[1].m_nameIndex == 1);
  assert (p1.m_links.m_elementRefs[2].m_elementIndex == 11);
  assert (p1.m_links.m_elementRefs[2].m_nameIndex == 0);
  assert (p1.m_links.m_elementRefs[3].m_elementIndex == 21);
  assert (p1.m_links.m_elementRefs[3].m_nameIndex == 1);
  assert (p1.m_links.m_elementRefs[4].m_elementIndex == 31);
  assert (p1.m_links.m_elementRefs[4].m_nameIndex == 2);
...
{
  MsgStream log (0, "test");
  NavigableVectorCnv_p1<VNIpar> cnv;
  VNIpar vni1;
  vni1.resize(2);
  ...
  NavigableVectorCnv_p1<VNIpar>::PersNavigableVector_t p1;
  cnv.transToPers (vni1, p1, log);
