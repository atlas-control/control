/**
 * @file CxxUtils/src/seal_signal.cxx
...
/** Suspend the thread waiting for signal @a sig at most @a msecs
    ...
    a signal is delivered.  Otherwise waits up to the specified time
    ...
    calling this function; if a handler is registered, it won't be
...
/** Suspend the thread waiting for signals specified by @a mask for at
    ...
    default), waits until a signal is delivered.  Otherwise waits up to
    ...
    calling one) and not be ignored before calling this function; if a
...
/** Install default handler for fatal signals.
    ...
    after some other library has meddled with the handlers.
...
/** Utility function to dump the signal info descriptor for signal @a
    sig, as obtained for instance through signal handler parameters or
