...
StatusCode AthenaSharedMemoryTool::stop() {
   ...
   if (m_isClient) {
      ATH_MSG_INFO("Client stop() inform Server");
      void** target = 0;
      size_t nbytes = 0;
      return(getObject("stop", target, nbytes));
   }
   return(StatusCode::SUCCESS);
}
...
StatusCode AthenaSharedMemoryTool::makeClient(int num) {
   ...
   if (m_isClient && num == 0) {
      void** target = 0;
      size_t nbytes = 0;
      getObject("stop", target, nbytes).ignore();
   }
   if (!m_isClient) {
      m_isClient = true;
      boost::interprocess::shared_memory_object shm(boost::interprocess::open_only,
   ...
   if (num > 0) {
      void** target = 0;
      size_t nbytes = 0;
      getObject("start", target, nbytes).ignore();
   }
   return(StatusCode::SUCCESS);
}
      ...
      if (evtH->evtProcessStatus == ShareEventHeader::LOCKED) {
         *tokenString = new char[maxTokenLength];
         ...
         if (strncmp(*tokenString, "start", 5) == 0) {
            ATH_MSG_INFO("Server lockObject() got start from: " << num);
            m_dataClients.insert(num);
            evtH->evtProcessStatus = ShareEventHeader::UNLOCKED;
            return(StatusCode::RECOVERABLE);
         }
         if (strncmp(*tokenString, "stop", 4) == 0) {
            ATH_MSG_INFO("Server lockObject() got stop from: " << num);
