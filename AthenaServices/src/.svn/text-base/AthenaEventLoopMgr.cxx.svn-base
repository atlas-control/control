  ...
  { // Deal with the case when an EventSelector is provided
    ...
    if ( pAttrList != 0) { // Try making EventID-only EventInfo object from in-file TAG
      try {
        ...
        pEvent = new EventInfo(new EventID(runNumber, eventNumber, eventTime, eventTimeNS, lumiBlock, bunchId), 0);
      } catch (...) {
...
*/
  ...
  else 
  {
    ...
    auto pEventPtr = CxxUtils::make_unique<EventInfo>
      (new EventID(0,m_nevt), new EventType());
