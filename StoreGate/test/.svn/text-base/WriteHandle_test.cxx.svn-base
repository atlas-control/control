...
public:
  ...
  void setStore (MyObjAux* store) {aux = store; }
  int x;
...
void test1()
{
...

  {
    ...
    EXPECT_EXCEPTION (SG::ExcHandleInitError, SG::WriteHandle<MyObj> h4 (k4));
  }
  ...
  assert (h6.store() == "OtherStore");

  {
    SG::WriteHandleKey<MyObj> k7 ("asd", "BazSvc");
    EXPECT_EXCEPTION (SG::ExcHandleInitError, SG::WriteHandle<MyObj> h7 (k7, ctx5));
  }
}
...
void test8()
{
  ...
  EXPECT_EXCEPTION (SG::ExcHandleInitError, SG::makeHandle (k2));
  ...
  assert (h3.mode() == Gaudi::DataHandle::Writer);
  assert (h3.store() == "OtherStore");

  SG::WriteHandleKey<MyObj> k4 ("asd", "BazSvc");
  EXPECT_EXCEPTION (SG::ExcHandleInitError, SG::makeHandle (k4, ctx));
}
